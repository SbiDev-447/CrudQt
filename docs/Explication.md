...
- **Consultas preparadas donde hay datos del usuario**: de las 17 sentencias SQL del proyecto, 11 usan `prepare()` + `bindValue()` — login, chequeos de duplicado, seed, INSERTs, UPDATEs y DELETEs. Las 6 restantes pasan SQL **literal** a `exec()` o al constructor de `QSqlQuery` (los dos `CREATE TABLE`, el `ALTER TABLE`, el `PRAGMA table_info`, el `UPDATE` del rol y el listado de administradores), y ninguna lleva datos del usuario. La regla que esto enseña es más útil que "usa siempre prepared statements": **`exec()` es correcto para SQL literal; lo peligroso sería construir una cadena con datos del usuario dentro, y eso no ocurre en ninguna de las 17.** Ningún dato del usuario se concatena a SQL: siempre viaja como valor ligado tras `:`.
- **Hash sin salt**: SHA-256 hex es correcto para aprender el patrón (guardar derivado, nunca en claro, comparar derivado) pero **no es suficiente para producción**: sin salt, dos usuarios con la misma contraseña tienen el mismo hash y las tablas rainbow aceleran la reversión. Nota educativa: en un sistema real hay que migrar a PBKDF2 (`QPasswordDigestor::deriveKeyPbkdf2` en Qt) o Argon2/bcrypt, con salt aleatorio por usuario. Ojo con el nombre: en Qt 6 esa función ya no se llama `deriveKey` —se dividió en `deriveKeyPbkdf1` y `deriveKeyPbkdf2`— y `QPasswordDigestor` vive en el módulo **Network**, no en Core, así que usarla exigiría añadir `Network` al `find_package` de `CMakeLists.txt`. El propio README ya hace esta advertencia.
